...
%         end
        ...
        %change in sinter & pellet
        for i=1:nobj
            fprintf(fileID,'%f %f %f %f %f %f %f %f %f %f \n',stats.EquivDiameter(i)/scale,stats.MajorAxisLength(i)/scale,stats.MinorAxisLength(i)/scale,stats.Perimeter(i)/scale,RollFricEcc1(i),Fmin/scale,Fmax/scale,inner/scale , outer/scale,stats.Orientation(i) );
        end
